<script src="<?php echo e($JQuerry); ?>"></script>
<script>
	window.addEventListener('load', function() {
		var handleFieldBillStatus = '<?php echo e($handleFieldBillStatus); ?>';
		var listBillStatuses = JSON.parse('<?php echo e($listBillStatuses); ?>'.replace(/&quot;/g,'"'));
		var billStatus = getValue(handleFieldBillStatus);
		genListItems();
		setListenerGenBill();
		setListenerApply();
		setListenerRefund();
		if(JSON.parse('<?php echo e($listNDSVal); ?>'.replace(/&quot;/g,'"')).indexOf(<?php echo $oneNDS===NULL?"null":$oneNDS?>) != -1)
			genBill();
		else
			$('.ck-list-items').show();
	});
	...
	function genListItems() {
		...
		$("tbody.ck-body-list-items").append(`
			<tr class="ck-order-item">
				<td>
					<?php echo e($labelSendBillOnEmail); ?> ` + window.order_info.client.email +`
				</td>
				<td class="check-box">
			...
			<tr class="ck-order-item">
				<td>
					<?php echo e($labelSendBillOnSMS); ?> ` + window.order_info.client.phone +`
				</td>
				<td class="check-box">
			...
			<tr class="ck-order-item">
				<td colspan="2" class="button-gen-bill">
					<button class="gen-bill"><?php echo e($buttonGenBill); ?></button>
				</td>
			</tr>
		`);
	}
	...
	function getSelectNDS() {
		var select = '<select>';
		var listNDS = JSON.parse('<?php echo e($listNDSVal); ?>'.replace(/&quot;/g,'"'));
	...
	function getNameNDS(key) {
		...
		switch(key) {
		  case null:  
		  	title = '<?php echo e($titleNDSNull); ?>';
		    break;
		  case 0:  
		  	title = '<?php echo e($titleNDS0); ?>';
		    break;
		  case 10:  
		  	title = '<?php echo e($titleNDS10); ?>';
		    break;
		  case 18:  
		  	title = '<?php echo e($titleNDS18); ?>';
		    break;
		  case 110:  
		  	title = '<?php echo e($titleNDS110); ?>';
		    break;
		  case 118:  
		  	title = '<?php echo e($titleNDS118); ?>';
		    break;
		}
		return title;
	}
	...
	function getIdDNS(str) {
		var id; 
		switch(str) {
		  case '<?php echo e($titleNDSNull); ?>':  
		  	id = null;
		    break;
		  case '<?php echo e($titleNDS0); ?>':  
		  	id = 0;
		    break;
		  case '<?php echo e($titleNDS10); ?>':  
		  	id = 10;
		    break;
		  case '<?php echo e($titleNDS18); ?>':  
		  	id = 18;
		    break;
		  case '<?php echo e($titleNDS110); ?>':  
		  	id = 110;
		    break;
		  case '<?php echo e($titleNDS118); ?>':  
		  	id = 118;
		    break;
		}
		return id;
	...
	function showLoadPicture()
	{
		...
		$("div.ck-load-images").append('<img class="ck-load-images" src="<?php echo e($loadImage); ?>">');
	}
	...
	function showApplyButton() {
		var statusBill = getValue('<?php echo e($handleFieldBillStatus); ?>');
		$("button.ck-apply-bill").hide();
		$("button.ck-apply-again-bill").hide();
	...
	function addNds() {
		window.order_info.order_lines.forEach(function(item, i){
			if(i >= 0 && i <= window.order_info.order_lines.length-1) {
				if(false/*JSON.parse('<?php echo e($listNDSVal); ?>'.replace(/&quot;/g,'"')).indexOf(<?php echo e($oneNDS); ?>) == -1*/)
